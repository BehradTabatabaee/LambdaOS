...
static int heap_validate_table(void *start, void *end, struct heap_table *table)
{
    ...
    size_t total_blocks = table_size / LAMBDAOS_HEAP_BLOCK_SIZE;
...
static bool heap_validate_alignment(void *start)
{
    return (((unsigned int)start % LAMBDAOS_HEAP_BLOCK_SIZE) == 0);
}
...
{
    if ((val % LAMBDAOS_HEAP_BLOCK_SIZE) == 0)
    {
        return val;
    }

    val = (val - (val % LAMBDAOS_HEAP_BLOCK_SIZE));
    val += LAMBDAOS_HEAP_BLOCK_SIZE;
    return val;
}
...
{
    return heap->start_address + (block * LAMBDAOS_HEAP_BLOCK_SIZE);
}
...
{
    return ((int)(address - heap->start_address)) / LAMBDAOS_HEAP_BLOCK_SIZE;
}

void *heap_malloc(struct heap *heap, size_t size)
{
    size_t aligned_size = heap_align_value_to_upper(size);
    uint32_t total_blocks = aligned_size / LAMBDAOS_HEAP_BLOCK_SIZE;
