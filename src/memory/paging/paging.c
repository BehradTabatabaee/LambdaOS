...
{
    // an array of addresses (uint32_t), used to fill the page directory
    // with addresses of page tables
    uint32_t *directory = kzalloc(sizeof(uint32_t) * PAGING_TOTAL_ENTRIES_PER_TABLE);
    int offset = 0;
    for (int i = 0; i < PAGING_TOTAL_ENTRIES_PER_TABLE; i++)
    {
        // an array of addresses (uint32_t), used to fill a page table
...
int paging_set(uint32_t *directory, void *virtual_address, uint32_t val)
{
    ...
    uint32_t *table = (uint32_t *)(entry & 0xfffff000); // entry's table address
